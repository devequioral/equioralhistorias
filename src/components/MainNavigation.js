...
export default function MainNavigation({
  ...
  const [showModalCalendar, setShowModalCalendar] = React.useState(0);
  useEffect(() => {
    if (showModalCalendar > 0) onShowCalendar(showModalCalendar);
  }, [showModalCalendar]);
    ...
    <div className={`${styles.MainNavigation}`}>
      ...
      {user && user?.role === 'admin' && (
        <Button
          ...
          className="btn-menu"
          onClick={() => {
            setShowModalCalendar((c) => c + 1);
          }}
          startContent={
            <Image
              src={`/assets/images/theme-light/icon-calendar.svg`}
              width={24}
              height={24}
              alt="Calendar"
            />
